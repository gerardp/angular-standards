...
- **v4** → the required path on Spartan 1.x. `@spartan-ng/brain@1.x` peers `tailwindcss >=4.0.0`, so installs on Tailwind v4 satisfy the peer check. Uses CSS-side configuration (`@theme` directive, layer imports).
- **v3** → **fails the peer check on Spartan 1.x** (`tailwindcss >=4.0.0`), npm will warn/error and recent Helm components may render unstyled. The legacy v3 recipe below (Step 6) is preserved only for older Spartan installs already locked to v3; new projects must be on v4.
- **Tailwind not installed yet** → install **v4** using Tailwind's official docs. Also ensure Spartan's other peers are present: `luxon`, `clsx`, `tw-animate-css`.
